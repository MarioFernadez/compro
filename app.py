...
def page_historial(user):
    ...
    # Si es admin, queremos ver el username también
    if user.role == "admin":
        with SessionLocal() as db:
        ...
        if st.button("🗑️ Eliminar"):
            with SessionLocal() as db:
                ...
                if user.role != "admin" and rec.user_id != user.id:
                    ...
                    return
                # borrado físico del archivo (best-effort)
                try:
    ...
    # Editor
    if "edit_rec_id" in st.session_state:
        st.divider()
        ...
        with st.form("edit_form"):
            ...
            currency = st.selectbox("Moneda", ["", "ARS", "PYG"], index=["", "ARS", "PYG"].index(payload.get("currency","") if payload.get("currency","") in ["ARS","PYG"] else ""))
        ...
        if ok:
            ...
            del st.session_state["edit_rec_id"]
    ...
    export_df = df.copy()
    # Trabajador: solo sus datos (ya filtrado). Admin: exporta todo.
    # Orden bonito:
